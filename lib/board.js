...
export class Board {
    ...
    constructor(width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                row.push(cell);
                this.drawCell(cell);
                this.#cellsToLookForNexTick.add(`${cell.x},${cell.y}`);
            }
            this.#cells.push(row);
        }
        for (let y = 0; y < height; y++) {
    ...
    tick() {
        ...
        const dataSlices = [];
        this.#cellsToLookForNexTick.forEach((c) => {
    ...
    async callTick(interval) {
        ...
        this.tick();
        this.#cellsToLookForNexTick = new Set();
        promises.forEach((promise) => {
            promise.then((data) => {
                if (data) {
                    ...
                    cellsForNextTick.forEach((c) => {
                        // const [x,y] = c.split(',').map(Number);
                        this.#cellsToLookForNexTick.add(c);
                    });
                }
            });
        });
